...
# install snscrape using: pip3 install --upgrade git+https://github.com/JustAnotherArchivist/snscrape.git
...
def get_user_tweets_without_retweets(username: str, max_tweets_num: int) -> List:
    try:
...
if __name__ == '__main__':
    ...
    for index, row in data_frame.iterrows():
        if pd.isna(row['twitter_url']):
            print(
                f"Skipping {row['person_name']}'s twitter as there is no twitter link")
            continue
